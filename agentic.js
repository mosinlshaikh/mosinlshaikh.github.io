...
const startIntent=s=>{const q=canon(s);if(/(start my project|start project|client form|form (bhar|fill)|project (banana|banwana|chahiye)|website (banana|banwana|chahiye)|software (banana|banwana|chahiye)|app (banana|banwana|chahiye)|मुझे.+(वेबसाइट|सॉफ्टवेयर|ऐप)|प्रोजेक्ट.+(बनाना|चाहिए)|वेबसाइट.+(हवी|पाहिजे)|सॉफ्टवेअर.+(हवे|पाहिजे))/iu.test(q))return true;try{const lang=detectQueryLanguage(s),terms=lang&&MULTILINGUAL_TERMS[lang];return !!terms?.build?.some(term=>q.includes(String(term).toLowerCase()))}catch{return false}};
