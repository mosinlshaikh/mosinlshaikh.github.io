...
imageButton?.addEventListener('keydown',e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();fileInput?.click()}});
fileInput?.addEventListener('change',async()=>{const file=fileInput.files?.[0];if(!file)return;if(file.size>8*1024*1024){addMessage('Please choose an image smaller than 8 MB.');return}const wait=addMessage('Reading text locally from the image…','bot','thinking');imageButton.style.pointerEvents='none';imageButton.setAttribute('aria-busy','true');let worker;try{const tesseract=await import('https://cdn.jsdelivr.net/npm/tesseract.js@6.0.1/dist/tesseract.esm.min.js'),createWorker=tesseract.createWorker||tesseract.default?.createWorker;if(typeof createWorker!=='function')throw new Error('OCR module did not expose createWorker');worker=await createWorker(['eng','hin','mar'],undefined,{workerPath:'https://cdn.jsdelivr.net/npm/tesseract.js@6.0.1/dist/worker.min.js',corePath:'https://cdn.jsdelivr.net/npm/tesseract.js-core@6.0.0',langPath:'https://tessdata.projectnaptha.com/4.0.0_best'});const result=await worker.recognize(file);wait.remove();const text=clean(result.data.text);if(!text)throw new Error('No readable text');addMessage(`Image text found:\n${text}`);chatInput.value=text.slice(0,1000);chatInput.focus()}catch(e){wait.remove();console.error('TTRL local OCR failed',e);addMessage('I could not read clear text from this image. Try a sharper, straight image with good lighting, or type the requirement.')}finally{try{await worker?.terminate()}catch{}imageButton.style.pointerEvents='';imageButton.removeAttribute('aria-busy');fileInput.value=''}});
